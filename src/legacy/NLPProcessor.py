...
# Load the pre-trained BERT modeldir and tokenizer
# model_name = 'dbmdz/bert-large-cased-finetuned-conll03-english'
# # model_name = 'neuralmind/bert-large-portuguese-cased'
# # # model_name = 'Luciano/bert-base-portuguese-cased-finetuned-peticoes'
# tokenizer = AutoTokenizer.from_pretrained(model_name, do_lower_case=False)
# modeldir = AutoModelForTokenClassification.from_pretrained(model_name)
#
...
#
# # Get the modeldir's predictions for the token classifications
# outputs = modeldir(tokens_tensor)
#
# # Get the predicted labels and convert them to strings
# predicted_labels = torch.argmax(outputs.logits, axis=-1)
# predicted_labels = [modeldir.config.id2label[label_id] for label_id in predicted_labels[0].tolist()]
#
