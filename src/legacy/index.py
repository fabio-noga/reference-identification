...
import NLPProcessor
from src import Utils, ExtractionUtils, DbUtils, Grabber
import sys
import csv
...
def seeQuotes():
    i = 0
    files = []
    for file in os.listdir("../assets/doc_json"):
        i = i + 1
        print(str(i) + ". " + file)
        files.append(os.path.join("../assets/doc_json", file))
    option = input("Choose the file:")
    data = ExtractionUtils.getQuotesFromFileByPattern(files[int(option) - 1])
    for line in data:
        print(line)
...
def createQuotesForAll():
    ...
    for i, file in enumerate(os.listdir("../assets/doc_json")):
        data = ExtractionUtils.getQuotesFromFileByPattern(os.path.join("../assets/doc_json", file))
        # data = Utils.getQuotesFromFileByNameFromListFinal(os.path.join("doc_json", file))
        try:
            folder = "quotes_verificadas2"
            title = file
...
def createQuotesForOne():
    ...
    data = ExtractionUtils.getQuotesFromFileByNameFromList(os.path.join("../assets/doc_json", file))
    try:
...
def createQuotesForOneFinal():
    ...
    file = "doc_99A796.json"
    data = ExtractionUtils.getQuotesFromFileByNameFromListFinal(os.path.join("../assets/doc_json", file))
    try:
        folder = "quotes_compare"
        title = file
        with open(folder + "/" + title + "8.txt", "w", encoding='utf-8') as outfile:
